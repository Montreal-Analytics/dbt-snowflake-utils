...
create or replace function {{target.schema}}.udf_business_hours(start_datetime TIMESTAMP_NTZ, end_datetime TIMESTAMP_NTZ, country STRING, weekmask STRING)
returns number
language python
runtime_version = 3.8
...
def business_hours_py(start_datetime, end_datetime, country, weekmask):
    ...
    if start_datetime.date() == end_datetime.date():
        ...
        if end_datetime.date() in holiday_list or end_datetime.weekday() in omitted_dow: 
            ...
            # If end_datetime does not fall on holiday or weekend, calculate last days working hours 
...
def business_hours_py(start_datetime, end_datetime, country, weekmask):
    ...
    # Default values, weekmask Monday to Friday
...
create or replace function {{target.schema}}.udf_business_hours(start_datetime TIMESTAMP_NTZ, end_datetime TIMESTAMP_NTZ)
returns number
language python
runtime_version = 3.8
packages = ('numpy','holidays')
...
def business_hours_py(start_datetime, end_datetime):

    # Default values for country (America) and weekmask (M-F) 
    country = 'US'
    weekmask = '1111100'
