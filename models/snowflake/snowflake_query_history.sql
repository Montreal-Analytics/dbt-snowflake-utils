...
SELECT *
FROM (
{%- for i in range(var('snowflake:max_load_minutes', 4320)) -%}
  {%- if i % var('snowflake:minutes_per_batch', 30) == 0 -%}
      ...
      FROM (
        ...
        from table(information_schema.query_history(
          ...
          end_time_range_end=>dateadd(minute, {{ var('snowflake:minutes_per_batch', 4320) }}, to_timestamp_ltz(dateadd(minute, {{i}}, $run_start_time))), RESULT_LIMIT => 10000))) qh
      {%- if var('snowflake:max_load_minutes', 30) - i > var('snowflake:minutes_per_batch', 30) %}
        UNION ALL
      {% endif -%}
  {%-endif-%}
{%- endfor %}
